...
-define(FILEPATH, "src\\hello.gleam").
-export([handle_message/2, main/0]).
-export_type([message/0]).

-type message() :: {add, integer()} |
    {get, gleam@erlang@process:subject(integer())}.

-file("src\\hello.gleam", 25).
...
handle_message(State, Message) ->
    case Message of
        ...
        {get, Reply} ->
            gleam@otp@actor:send(Reply, State),
            gleam@otp@actor:continue(State)
    end.

-file("src\\hello.gleam", 4).
-spec main() -> nil.
main() ->
    Actor@1 = case begin
        _pipe = gleam@otp@actor:new(0),
    ...
    gleam@otp@actor:send(erlang:element(3, Meh@1), {add, 4}),
    _assert_subject = gleam@otp@actor:call(
        erlang:element(3, Actor@1),
        20,
