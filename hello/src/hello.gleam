import argv
import gleam/erlang/process.{type Subject} as erlang
import gleam/int
import gleam/list
...
pub fn get_process_cpu_time(pid) {
  let info = erlang.
  case erlang.get_property(info, "cpu_time") {
    Some(cpu_time) -> Ok(cpu_time)
    _ -> Error("Could not retrieve cpu_time")
  }
}
