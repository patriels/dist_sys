...
-opaque subject(DPY) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DPY)}.

-type name(DPZ) :: any() | {gleam_phantom, DPZ}.

-type do_not_leak() :: any().

-type selector(DQA) :: any() | {gleam_phantom, DQA}.
...
-spec 'receive'(subject(DQY), integer()) -> {ok, DQY} | {error, nil}.
...
-spec receive_forever(subject(DRG)) -> DRG.
...
-spec selector_receive(selector(DRK), integer()) -> {ok, DRK} | {error, nil}.
...
-spec selector_receive_forever(selector(DRO)) -> DRO.
...
-spec map_selector(selector(DRQ), fun((DRQ) -> DRS)) -> selector(DRS).
map_selector(A, B) ->
...
-spec merge_selector(selector(DRU), selector(DRU)) -> selector(DRU).
merge_selector(A, B) ->
...
-spec select_map(selector(DSF), subject(DSH), fun((DSH) -> DSF)) -> selector(DSF).
...
-spec select(selector(DSB), subject(DSB)) -> selector(DSB).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DSP)
) -> selector(DSP).
...
-spec select_other(selector(DST), fun((gleam@dynamic:dynamic_()) -> DST)) -> selector(DST).
...
-spec deselect(selector(DSK), subject(any())) -> selector(DSK).
...
-spec select_specific_monitor(selector(DTF), monitor(), fun((down()) -> DTF)) -> selector(DTF).
...
-spec select_monitors(selector(DTI), fun((down()) -> DTI)) -> selector(DTI).
...
-spec select_trapped_exits(selector(DRY), fun((exit_message()) -> DRY)) -> selector(DRY).
...
-spec deselect_specific_monitor(selector(DTL), monitor()) -> selector(DTL).
...
-spec send_after(subject(DUH), integer(), DUH) -> timer().
...
-spec perform_call(
    subject(DTO),
    fun((subject(DTQ)) -> DTO),
    fun((selector(DTQ)) -> {ok, DTQ} | {error, nil})
) -> DTQ.
...
-spec call(subject(DTV), integer(), fun((subject(DTX)) -> DTV)) -> DTX.
...
-spec call_forever(subject(DTZ), fun((subject(DUB)) -> DTZ)) -> DUB.
