...
-spec be_ok({ok, FQM} | {error, any()}) -> FQM.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, FQR}) -> FQR.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(FQU)) -> FQU.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
