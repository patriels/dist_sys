...
-spec map_first({CPC, CPD}, fun((CPC) -> CPE)) -> {CPE, CPD}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CPF, CPG}, fun((CPG) -> CPH)) -> {CPF, CPH}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
