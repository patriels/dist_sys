...
-spec map({ok, CPT} | {error, CPU}, fun((CPT) -> CPX)) -> {ok, CPX} |
    {error, CPU}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CQA} | {error, CQB}, fun((CQB) -> CQE)) -> {ok, CQA} |
    {error, CQE}.
...
-spec flatten({ok, {ok, CQH} | {error, CQI}} | {error, CQI}) -> {ok, CQH} |
    {error, CQI}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CQP} | {error, CQQ}, fun((CQP) -> {ok, CQT} | {error, CQQ})) -> {ok,
        CQT} |
    {error, CQQ}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CQY} | {error, CQZ}, fun((CQY) -> {ok, CRC} | {error, CQZ})) -> {ok,
        CRC} |
    {error, CQZ}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CRH} | {error, any()}, CRH) -> CRH.
...
-spec lazy_unwrap({ok, CRL} | {error, any()}, fun(() -> CRL)) -> CRL.
...
-spec unwrap_error({ok, any()} | {error, CRQ}, CRQ) -> CRQ.
...
-spec unwrap_both({ok, CRT} | {error, CRT}) -> CRT.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CRW} | {error, CRX}, {ok, CRW} | {error, CRX}) -> {ok, CRW} |
    {error, CRX}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CSE} | {error, CSF}, fun(() -> {ok, CSE} | {error, CSF})) -> {ok,
        CSE} |
    {error, CSF}.
...
-spec all(list({ok, CSM} | {error, CSN})) -> {ok, list(CSM)} | {error, CSN}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src\\gleam\\result.gleam", 367).
-spec partition_loop(list({ok, CTB} | {error, CTC}), list(CTB), list(CTC)) -> {list(CTB),
    list(CTC)}.
...
-spec partition(list({ok, CSU} | {error, CSV})) -> {list(CSU), list(CSV)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CTK}, CTN) -> {ok, CTN} | {error, CTK}.
...
-spec replace_error({ok, CTQ} | {error, any()}, CTU) -> {ok, CTQ} | {error, CTU}.
...
-spec values(list({ok, CTX} | {error, any()})) -> list(CTX).
values(Results) ->
