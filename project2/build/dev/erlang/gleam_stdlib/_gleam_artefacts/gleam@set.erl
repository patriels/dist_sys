...
-opaque set(CZC) :: {set, gleam@dict:dict(CZC, list(nil))}.
...
-spec contains(set(CZN), CZN) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CZP), CZP) -> set(CZP).
delete(Set, Member) ->
...
-spec fold(set(CZY), DAA, fun((DAA, CZY) -> DAA)) -> DAA.
...
-spec filter(set(DAB), fun((DAB) -> boolean())) -> set(DAB).
...
-spec drop(set(DAI), list(DAI)) -> set(DAI).
drop(Set, Disallowed) ->
...
-spec take(set(DAM), list(DAM)) -> set(DAM).
take(Set, Desired) ->
...
-spec intersection(set(DAZ), set(DAZ)) -> set(DAZ).
...
-spec difference(set(DBD), set(DBD)) -> set(DBD).
...
-spec is_subset(set(DBH), set(DBH)) -> boolean().
...
-spec is_disjoint(set(DBK), set(DBK)) -> boolean().
...
-spec each(set(DBR), fun((DBR) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CZK), CZK) -> set(CZK).
insert(Set, Member) ->
...
-spec from_list(list(CZV)) -> set(CZV).
from_list(Members) ->
...
-spec map(set(DAE), fun((DAE) -> DAG)) -> set(DAG).
map(Set, Fun) ->
...
-spec union(set(DAQ), set(DAQ)) -> set(DAQ).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(DBN), set(DBN)) -> set(DBN).
