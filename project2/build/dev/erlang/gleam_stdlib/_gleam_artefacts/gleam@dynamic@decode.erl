...
-spec run(gleam@dynamic:dynamic_(), decoder(BYK)) -> {ok, BYK} |
...
-file("src\\gleam\\dynamic\\decode.gleam", 874).
?DOC(
...
-spec map(decoder(CCI), fun((CCI) -> CCK)) -> decoder(CCK).
...
-file("src\\gleam\\dynamic\\decode.gleam", 883).
...
-file("src\\gleam\\dynamic\\decode.gleam", 921).
?DOC(
...
-spec then(decoder(CCU), fun((CCU) -> decoder(CCW))) -> decoder(CCW).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-file("src\\gleam\\dynamic\\decode.gleam", 951).
?DOC(
...
-spec one_of(decoder(CCZ), list(decoder(CCZ))) -> decoder(CCZ).
...
-file("src\\gleam\\dynamic\\decode.gleam", 1047).
?DOC(
...
-spec recursive(fun(() -> decoder(CDP))) -> decoder(CDP).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-file("src\\gleam\\dynamic\\decode.gleam", 852).
?DOC(
...
-spec optional(decoder(CCE)) -> decoder(gleam@option:option(CCE)).
optional(Inner) ->
...
-file("src\\gleam\\dynamic\\decode.gleam", 907).
?DOC(
...
-spec collapse_errors(decoder(CCR), binary()) -> decoder(CCR).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CDL} | {error, CDL})
) -> decoder(CDL).
...
-file("src\\gleam\\dynamic\\decode.gleam", 806).
-spec fold_dict(
    {gleam@dict:dict(CBQ, CBR), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CBQ, list(decode_error())}),
...
-file("src\\gleam\\dynamic\\decode.gleam", 786).
?DOC(
...
-spec dict(decoder(CBJ), decoder(CBL)) -> decoder(gleam@dict:dict(CBJ, CBL)).
dict(Key, Value) ->
...
-file("src\\gleam\\dynamic\\decode.gleam", 754).
?DOC(
...
-spec list(decoder(CAX)) -> decoder(list(CAX)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BZG, list(decode_error())}, list(any())) -> {BZG,
    list(decode_error())}.
...
-spec index(
    list(BYU),
    list(BYU),
    fun((gleam@dynamic:dynamic_()) -> {BYX, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BYU)) -> {BYX, list(decode_error())})
) -> {BYX, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BYF), fun((BYF) -> decoder(BYH))) -> decoder(BYH).
...
-spec at(list(any()), decoder(BYR)) -> decoder(BYR).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BZP), fun((BZP) -> decoder(BZR))) -> decoder(BZR).
...
-spec optional_field(any(), BZV, decoder(BZV), fun((BZV) -> decoder(BZX))) -> decoder(BZX).
...
-spec optionally_at(list(any()), CAC, decoder(CAC)) -> decoder(CAC).
