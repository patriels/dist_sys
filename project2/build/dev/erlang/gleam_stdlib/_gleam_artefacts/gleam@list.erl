...
-file("src\\gleam\\list.gleam", 1015).
?DOC(
...
-file("src\\gleam\\list.gleam", 1098).
...
-file("src\\gleam\\list.gleam", 1125).
?DOC(
...
-file("src\\gleam\\list.gleam", 1164).
...
-file("src\\gleam\\list.gleam", 1157).
?DOC(
...
-file("src\\gleam\\list.gleam", 1187).
...
-file("src\\gleam\\list.gleam", 1273).
?DOC(
...
-file("src\\gleam\\list.gleam", 1421).
?DOC(
...
-file("src\\gleam\\list.gleam", 1396).
...
-file("src\\gleam\\list.gleam", 1340).
?DOC(
...
-file("src\\gleam\\list.gleam", 1211).
?DOC(
...
-file("src\\gleam\\list.gleam", 1514).
...
-file("src\\gleam\\list.gleam", 1547).
...
-file("src\\gleam\\list.gleam", 1543).
?DOC(
...
-file("src\\gleam\\list.gleam", 1576).
?DOC(
...
-file("src\\gleam\\list.gleam", 1623).
?DOC(
...
-file("src\\gleam\\list.gleam", 1654).
?DOC(
...
-file("src\\gleam\\list.gleam", 1690).
?DOC(
    " Given a list of 2-element tuples, finds the first tuple that has a given\n"
...
-file("src\\gleam\\list.gleam", 1756).
?DOC(
...
-spec partition_loop(list(BHC), fun((BHC) -> boolean()), list(BHC), list(BHC)) -> {list(BHC),
    list(BHC)}.
...
-file("src\\gleam\\list.gleam", 1808).
?DOC(
...
-spec window_loop(list(list(AOY)), list(AOY), integer()) -> list(list(AOY)).
...
-file("src\\gleam\\list.gleam", 1888).
?DOC(
...
-spec window(list(AOU), integer()) -> list(list(AOU)).
window(List, N) ->
    case N =< 0 of
        true ->
            [];
...
-file("src\\gleam\\list.gleam", 1918).
?DOC(
...
-spec window_by_2(list(APE)) -> list({APE, APE}).
window_by_2(List) ->
...
-spec drop_while(list(APH), fun((APH) -> boolean())) -> list(APH).
...
-file("src\\gleam\\list.gleam", 1961).
-spec take_while_loop(list(APN), fun((APN) -> boolean()), list(APN)) -> list(APN).
...
-file("src\\gleam\\list.gleam", 1954).
?DOC(
...
-spec take_while(list(APK), fun((APK) -> boolean())) -> list(APK).
take_while(List, Predicate) ->
    take_while_loop(List, Predicate, []).

-file("src\\gleam\\list.gleam", 1993).
-spec chunk_loop(list(APW), fun((APW) -> APY), APY, list(APW), list(list(APW))) -> list(list(APW)).
...
-spec chunk(list(APR), fun((APR) -> any())) -> list(list(APR)).
chunk(List, F) ->
    case List of
        [] ->
            [];
...
-file("src\\gleam\\list.gleam", 2038).
...
-spec sized_chunk(list(AQE), integer()) -> list(list(AQE)).
sized_chunk(List, Count) ->
    sized_chunk_loop(List, Count, Count, [], []).

-file("src\\gleam\\list.gleam", 2082).
?DOC(
...
-spec reduce(list(AQP), fun((AQP, AQP) -> AQP)) -> {ok, AQP} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(AQX), AQZ, list(AQZ), fun((AQZ, AQX) -> AQZ)) -> list(AQZ).
...
-file("src\\gleam\\list.gleam", 2098).
?DOC(
...
-spec scan(list(AQT), AQV, fun((AQV, AQT) -> AQV)) -> list(AQV).
scan(List, Initial, Fun) ->
    scan_loop(List, Initial, [], Fun).

-file("src\\gleam\\list.gleam", 2139).
?DOC(
...
-spec last(list(ARC)) -> {ok, ARC} | {error, nil}.
last(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec combinations(list(ARG), integer()) -> list(list(ARG)).
...
-file("src\\gleam\\list.gleam", 2186).
-spec combination_pairs_loop(list(ARN), list({ARN, ARN})) -> list({ARN, ARN}).
...
-file("src\\gleam\\list.gleam", 2182).
?DOC(
...
-spec combination_pairs(list(ARK)) -> list({ARK, ARK}).
combination_pairs(Items) ->
    combination_pairs_loop(Items, []).

-file("src\\gleam\\list.gleam", 2241).
-spec take_firsts(list(list(ASH)), list(ASH), list(list(ASH))) -> {list(ASH),
    list(list(ASH))}.
...
-spec transpose_loop(list(list(ASA)), list(list(ASA))) -> list(list(ASA)).
...
-file("src\\gleam\\list.gleam", 2224).
?DOC(
...
-spec transpose(list(list(ARV))) -> list(list(ARV)).
transpose(List_of_lists) ->
    transpose_loop(List_of_lists, []).

-file("src\\gleam\\list.gleam", 2206).
?DOC(
...
-spec interleave(list(list(ARR))) -> list(ARR).
interleave(List) ->
    _pipe = transpose(List),
    lists:append(_pipe).

-file("src\\gleam\\list.gleam", 2274).
-spec shuffle_pair_unwrap_loop(list({float(), AST}), list(AST)) -> list(AST).
...
-file("src\\gleam\\list.gleam", 2282).
-spec do_shuffle_by_pair_indexes(list({float(), ASX})) -> list({float(), ASX}).
...
-spec max_loop(list(ATH), fun((ATH, ATH) -> gleam@order:order()), ATH) -> ATH.
...
-spec max(list(ATA), fun((ATA, ATA) -> gleam@order:order())) -> {ok, ATA} |
    {error, nil}.
max(List, Compare) ->
    case List of
        [] ->
...
-file("src\\gleam\\list.gleam", 2395).
-spec build_reservoir_loop(
    list(ATW),
    integer(),
    gleam@dict:dict(integer(), ATW)
) -> {gleam@dict:dict(integer(), ATW), list(ATW)}.
...
-file("src\\gleam\\list.gleam", 2391).
?DOC(
...
-spec build_reservoir(list(ATR), integer()) -> {gleam@dict:dict(integer(), ATR),
    list(ATR)}.
build_reservoir(List, N) ->
    build_reservoir_loop(List, N, maps:new()).

-file("src\\gleam\\list.gleam", 2379).
...
-file("src\\gleam\\list.gleam", 2356).
-spec sample_loop(
    list(ATL),
    gleam@dict:dict(integer(), ATL),
    integer(),
    float()
) -> gleam@dict:dict(integer(), ATL).
...
-file("src\\gleam\\list.gleam", 2338).
?DOC(
...
-spec sample(list(ATI), integer()) -> list(ATI).
sample(List, N) ->
...
-file("src\\gleam\\list.gleam", 1842).
-spec permutation_zip(list(AOE), list(AOE), list(list(AOE))) -> list(list(AOE)).
...
-file("src\\gleam\\list.gleam", 1835).
?DOC(
