...
-type message(EAV) :: {message, EAV} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(EAW, EAX) :: {continue,
        EAW,
        gleam@option:option(gleam@erlang@process:selector(EAX))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(EAY, EAZ) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        EAY,
        gleam@erlang@process:selector(message(EAZ)),
        gleam@otp@system:debug_state(),
        fun((EAY, EAZ) -> next(EAY, EAZ))}.

-type started(EBA) :: {started, gleam@erlang@process:pid_(), EBA}.

-opaque initialised(EBB, EBC, EBD) :: {initialised,
        EBB,
        gleam@option:option(gleam@erlang@process:selector(EBC)),
        EBD}.

-opaque builder(EBE, EBF, EBG) :: {builder,
        fun((gleam@erlang@process:subject(EBF)) -> {ok,
                initialised(EBE, EBF, EBG)} |
            {error, binary()}),
        integer(),
        fun((EBE, EBF) -> next(EBE, EBF)),
        gleam@option:option(gleam@erlang@process:name(EBF))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(EBH) :: {ack, {ok, EBH} | {error, binary()}} |
...
-spec continue(EBM) -> next(EBM, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(EBY, EBZ), gleam@erlang@process:selector(EBZ)) -> next(EBY, EBZ).
...
-spec initialised(ECF) -> initialised(ECF, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(ECK, any(), ECM),
    gleam@erlang@process:selector(ECQ)
) -> initialised(ECK, ECQ, ECM).
...
-spec returning(initialised(ECV, ECW, any()), EDB) -> initialised(ECV, ECW, EDB).
...
-spec new(EDF) -> builder(EDF, EDG, gleam@erlang@process:subject(EDG)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(EDL)) -> {ok, initialised(EDN, EDL, EDO)} |
        {error, binary()})
) -> builder(EDN, EDL, EDO).
...
-spec on_message(builder(EDX, EDY, EDZ), fun((EDX, EDY) -> next(EDX, EDY))) -> builder(EDX, EDY, EDZ).
...
-spec named(builder(EEI, EEJ, EEK), gleam@erlang@process:name(EEJ)) -> builder(EEI, EEJ, EEK).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(EEX))) -> gleam@erlang@process:selector(message(EEX)).
...
-spec receive_message(self(any(), EET)) -> message(EET).
...
-spec initialise_actor(
    builder(any(), any(), EFQ),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EFQ} | {error, binary()})
...
-spec start(builder(any(), any(), EGD)) -> {ok, started(EGD)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EGK), EGK) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EGM),
    integer(),
    fun((gleam@erlang@process:subject(EGO)) -> EGM)
) -> EGO.
