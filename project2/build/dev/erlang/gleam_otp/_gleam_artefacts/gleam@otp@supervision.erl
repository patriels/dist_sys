...
-type child_specification(EUA) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(EUA)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(EUB)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EUB).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(EUG)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EUG).
supervisor(Start) ->
...
-spec significant(child_specification(EUL), boolean()) -> child_specification(EUL).
...
-spec timeout(child_specification(EUO), integer()) -> child_specification(EUO).
timeout(Child, Ms) ->
...
-spec restart(child_specification(EUR), restart()) -> child_specification(EUR).
...
-spec map_data(child_specification(EUU), fun((EUU) -> EUW)) -> child_specification(EUW).
